...
public class MicrophoneEncoder implements Runnable {
    ...
    public MicrophoneEncoder(RecorderConfig config) throws IOException {
        init(config);
    }

    private void init(RecorderConfig config) throws IOException {
        mEncoderCore = new AudioEncoderCore(config.audioEncoderConfig.numChannels,
                config.audioEncoderConfig.bitRate,
                config.audioEncoderConfig.sampleRate,
                config.muxer);
        mMediaCodec = null;
        mThreadReady = false;
